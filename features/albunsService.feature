...
Scenario: Inserindo single no sistema
    Given o usuario de login "joydivision" está autenticado como "artist"
...
Scenario: inserindo um EP no sistema
    Given o usuario de login "kurt" está autenticado como "artist"
    When uma requisição "POST" é feita no endpoint "/albums"
    And o campo "genero" está preenchido com "Rock"
...
Scenario: falha ao atualizar música com campo nome vazio
    Given o usuario de login "rocker" está autenticado como "artist"
    And existe um lançamento com id "2" para esse usuario
